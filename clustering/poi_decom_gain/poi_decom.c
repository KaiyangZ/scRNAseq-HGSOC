...
SEXP
poi_decom_control_solve_R(SEXP X_R, SEXP t_A_R, SEXP B_R, SEXP max_iter_R, SEXP min_delta_R)
{
	int m, n, r, j;
	SEXP Y_R;
	double *S;
...
SEXP
poi_decom_control_label_R(SEXP L_R, SEXP X_R, SEXP t_A_R, SEXP B_R, SEXP g_R, SEXP s_R)
{
	int m, n, r, s, j, lab;
	double *S, res;
...
SEXP
poi_decom_control_centroid_R(SEXP t_A_R, SEXP t_B_R, SEXP g_R)
{
	int m, n, i;
	SEXP x_R;
...
SEXP
poi_cellDist_R(SEXP t_A_R, SEXP t_B_R, SEXP g_R, SEXP gw_R)
{
    ...
    */
	...
	if (!(isReal(g_R) && isVector(g_R)))
		error("'%s' must be a numeric vector", "g");
	if (!(isReal(gw_R) && isVector(gw_R)))
		error("'%s' must be a numeric vector", "gw");
	...
	if (!(length(g_R) == n))
		error("'%s' must be %d-by-1", "g", n);
    if (!(length(gw_R) == m ))        
        error("'%s' must be %d-by-1", "gw", m);

	/* Create output */
	d_R = PROTECT(allocMatrix(REALSXP, n, n));

    /* Zero output */
    memset( REAL(d_R), 0, n*n * sizeof(*REAL(d_R)) );
	
    /* Loop over genes */
	    /* Compute pair-wise distances of each pair of samples */
	#pragma omp parallel for private(j2, i) schedule(dynamic)
	for (j1 = 0; j1 < n; ++j1)
    	for (j2 = j1+1; j2 < n; ++j2)
			for (i = 0; i < m; ++i)
            {
                double a[2], b[2], g[2], lam, d, gwi;

                /* Pull data */
                a[0] = REAL(t_A_R)[j1 + i*n];
                a[1] = REAL(t_A_R)[j2 + i*n];
                b[0] = REAL(t_B_R)[j1 + i*n];
                b[1] = REAL(t_B_R)[j2 + i*n];
                g[0] = REAL(g_R)[j1];
                g[1] = REAL(g_R)[j2];
                gwi = REAL(gw_R)[i];

                /* Compute centroid & distances */
                lam = centroid_poi(a, b, g, 2);
                d = poi_dist(b[0], g[0] * (a[0] + lam)) + poi_dist(b[1], g[1] * (a[1] + lam));
/*
*Rprintf("lam = %g\n", lam);                
*Rprintf("a[0] + lam = %g, lam0 = %g, d0 = %g\n", a[0] + lam,
*g[0] * (a[0] + lam) ,poi_dist(b[0], g[0] * (a[0] + lam)) );
*Rprintf("a[1] + lam = %g, lam1 = %g, d1 = %g\n", a[1] + lam,
*g[1] * (a[1] + lam) ,poi_dist(b[1], g[1] * (a[1] + lam)) );
*/

                /* Save */
                REAL(d_R)[j2 + j1*n] += gwi * d;
                /*REAL(d_R)[j2 + j1*n] += d;*/
                 /*w[i ] = 1/sum( d[j1,j2,i] , (j1,j2) )*/
            }

    /* Fill lower triangular part */
    for (j1 = 0; j1 < n; ++j1)
        for (j2 = j1+1; j2 < n; ++j2)
            REAL(d_R)[j1 + j2*n] = REAL(d_R)[j2 + j1*n];
